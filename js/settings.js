...
const Settings = {
  ...
  // ---------- Theme (dark mode) ----------
  async initTheme() {
    let dark = await DB.getSetting('darkMode', null);
    if (dark === null) {
  ...
  applyTheme(dark) {
    ...
    try { localStorage.setItem('betterme-theme', dark ? 'dark' : 'light'); } catch (e) {}
    const meta = document.querySelector('meta[name="theme-color"]');
    if (meta) meta.setAttribute('content', dark ? '#121026' : '#4F46E5');
