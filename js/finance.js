...
const Finance = {
  ...
  renderMovimientos(wrap) {
    ...
    sorted.slice(0, 60).forEach(t => {
      ...
      const icons = { ingreso: Icon.trendUp(), gasto: Icon.cart(), ahorro: Icon.piggyBank(), deuda: Icon.fileText(), cobro: Icon.handshake() };
      const row = Utils.el(`
    ...
    const wrap = Utils.el(`
      <div>
        ...
        <div style="display:flex;flex-direction:column;gap:10px;">
          <button class="btn btn-secondary btn-block" data-t="ingreso"><span class="icon-inline" style="width:16px;height:16px;">${Icon.trendUp()}</span> Ingreso</button>
          <button class="btn btn-secondary btn-block" data-t="gasto"><span class="icon-inline" style="width:16px;height:16px;">${Icon.cart()}</span> Gasto</button>
          <button class="btn btn-secondary btn-block" data-t="ahorro"><span class="icon-inline" style="width:16px;height:16px;">${Icon.piggyBank()}</span> Ahorro</button>
          <button class="btn btn-secondary btn-block" data-t="deseo"><span class="icon-inline" style="width:16px;height:16px;">${Icon.sparkles()}</span> Deseo (wishlist)</button>
          <button class="btn btn-secondary btn-block" data-t="deuda"><span class="icon-inline" style="width:16px;height:16px;">${Icon.fileText()}</span> Deuda</button>
          <button class="btn btn-secondary btn-block" data-t="cobrar"><span class="icon-inline" style="width:16px;height:16px;">${Icon.handshake()}</span> Cuenta por cobrar</button>
        </div>
      </div>
    `);
