...
const Habits = {
  ...
  isCompletedOnDate(habit, iso) {
    ...
    if (habit.tipo === 'numerico') return habit.objetivo ? log.value >= habit.objetivo : log.value != null;
    return false;
  },
  ...
  render() {
    ...
    const ringWrap = document.getElementById('habits-hero-ring');
    if (ringWrap) ringWrap.innerHTML = Utils.ringSVG(pct, 26, 62);
  ...
  renderHabitItem(habit, iso) {
    ...
    const typeMeta = { sino: Icon.checkSquare(), contador: Icon.hash(), tiempo: Icon.clock(), numerico: Icon.barChart() };
    const item = Utils.el(`
      <div class="habit-item ${done ? 'completed' : ''}" style="--habit-color:${habit.color}">
        <div data-role="indicator"></div>
        <div class="habit-name-wrap" data-role="open">
          <div class="habit-name ${done ? 'done' : ''}">${Utils.escapeHtml(habit.nombre)}</div>
          <div class="habit-meta">
            <span class="habit-type-badge"><span class="icon-inline">${typeMeta[habit.tipo]}</span>${this.typeLabel(habit)}</span>
            ${streak.current > 0 ? `<span class="habit-streak"><span class="icon-inline">${Icon.flame()}</span>${streak.current}</span>` : ''}
          </div>
        </div>
    ...
    const indicatorSlot = item.querySelector('[data-role="indicator"]');
    const rightEl = item.querySelector('.habit-right');

    if (habit.tipo === 'sino') {
      const check = Utils.el(`<div class="habit-check ${done ? 'checked' : ''}">${done ? '✓' : ''}</div>`);
      check.onclick = (e) => { e.stopPropagation(); this.toggleSiNo(habit, iso); };
      indicatorSlot.replaceWith(check);
    } else if (habit.tipo === 'contador') {
      const log = this.logFor(habit.id, iso);
      const val = log ? log.value : 0;
      const pct = habit.objetivo ? Math.min(100, (val / habit.objetivo) * 100) : (val > 0 ? 100 : 0);
      const ring = Utils.el(`<div class="habit-ring">${Utils.ringSVG(pct)}</div>`);
      indicatorSlot.replaceWith(ring);
      rightEl.innerHTML = `
        <div class="habit-counter-controls">
          <button data-act="dec">−</button>
          <span class="habit-counter-val" data-act="custom" style="cursor:pointer;">${val}${habit.objetivo ? '/' + habit.objetivo : ''}</span>
          <button data-act="inc">+</button>
        </div>`;
      rightEl.querySelector('[data-act="inc"]').onclick = (e) => { e.stopPropagation(); this.incValue(habit, iso, 1); };
      rightEl.querySelector('[data-act="dec"]').onclick = (e) => { e.stopPropagation(); this.incValue(habit, iso, -1); };
      rightEl.querySelector('[data-act="custom"]').onclick = (e) => { e.stopPropagation(); this.promptAddCounterValue(habit, iso); };
    } else if (habit.tipo === 'tiempo') {
      const log = this.logFor(habit.id, iso);
      const val = log ? log.value : 0;
      const pct = habit.objetivo ? Math.min(100, (val / habit.objetivo) * 100) : (val > 0 ? 100 : 0);
      const ring = Utils.el(`<div class="habit-ring">${Utils.ringSVG(pct)}</div>`);
      ring.onclick = (e) => { e.stopPropagation(); this.promptTimeValue(habit, iso); };
      indicatorSlot.replaceWith(ring);
      rightEl.innerHTML = `<div class="habit-counter-val" style="color:var(--habit-color);font-weight:700;">${val} min</div>`;
    } else if (habit.tipo === 'numerico') {
      const log = this.logFor(habit.id, iso);
      const val = log ? log.value : null;
      const pct = habit.objetivo ? (val != null ? Math.min(100, (val / habit.objetivo) * 100) : 0) : (val != null ? 100 : 0);
      const ring = Utils.el(`<div class="habit-ring">${Utils.ringSVG(pct)}</div>`);
      ring.onclick = (e) => { e.stopPropagation(); this.promptNumericValue(habit, iso); };
      indicatorSlot.replaceWith(ring);
      rightEl.innerHTML = `<div class="habit-counter-val" style="color:var(--habit-color);font-weight:700;">${val != null ? val + ' ' + (habit.unidad||'') : 'Registrar'}</div>`;
    }
  ...
  async promptAddCounterValue(habit, iso) {
    if (Utils.isFuture(iso)) { Utils.toast('No puedes registrar fechas futuras'); return; }
    const val = prompt(`¿Cuánto quieres agregar a "${habit.nombre}"?${habit.unidad ? ' (' + habit.unidad + ')' : ''}`, '');
    if (val === null) return;
    const n = parseFloat(val);
    if (isNaN(n) || n <= 0) return;
    let log = this.logFor(habit.id, iso);
    if (!log) { log = { id: DB.uuid(), habitId: habit.id, date: iso, value: 0 }; this.logs.push(log); }
    log.value = Math.max(0, (log.value || 0) + n);
    await DB.put('habitLogs', log);
    this.render();
    if (habit.objetivo && log.value >= habit.objetivo) this.maybeOfferExpense(habit, iso);
  },
  ...
  async promptTimeValue(habit, iso) {
    if (Utils.isFuture(iso)) { Utils.toast('No puedes registrar fechas futuras'); return; }
    const val = prompt(`¿Cuántos minutos quieres agregar a "${habit.nombre}"?`, '');
    if (val === null) return;
  ...
  async promptNumericValue(habit, iso) {
    if (Utils.isFuture(iso)) { Utils.toast('No puedes registrar fechas futuras'); return; }
    const acumula = habit.acumulativo !== false;
    const label = acumula ? `¿Cuánto quieres agregar a "${habit.nombre}"?` : `${habit.nombre}`;
    const val = prompt(`${label}${habit.unidad ? ' (' + habit.unidad + ')' : ''}:`, '');
    if (val === null) return;
    const n = parseFloat(val);
    if (isNaN(n)) return;
    let log = this.logFor(habit.id, iso);
    if (!log) { log = { id: DB.uuid(), habitId: habit.id, date: iso, value: 0 }; this.logs.push(log); }
    log.value = acumula ? Math.max(0, (log.value || 0) + n) : n;
    await DB.put('habitLogs', log);
    this.render();
    if (habit.objetivo && log.value >= habit.objetivo) this.maybeOfferExpense(habit, iso);
  },
  ...
  openCreateForm(existing = null) {
    ...
    const wrap = Utils.el(`
      <div>
        ...
        <div class="field" id="hf-acumular-field">
          <label style="display:flex;align-items:center;gap:8px;text-transform:none;font-size:14px;"><input type="checkbox" id="hf-acumular" style="width:auto;"> Sumar los valores que registre durante el día (ej. 300ml + 300ml = 600ml)</label>
        </div>
        <div class="field">
    ...
    wrap.querySelector('#hf-acumular').checked = h.acumulativo !== false;
    ...
    [...tipoGroup.children].forEach(c => {
      ...
      c.onclick = () => { [...tipoGroup.children].forEach(x => x.classList.remove('active')); c.classList.add('active');
        ...
        wrap.querySelector('#hf-acumular-field').style.display = (c.dataset.v === 'numerico') ? 'block' : 'none';
      };
    });
    wrap.querySelector('#hf-unidad-field').style.display = (h.tipo === 'sino') ? 'none' : 'block';
    wrap.querySelector('#hf-acumular-field').style.display = (h.tipo === 'numerico') ? 'block' : 'none';
    ...
    wrap.querySelector('#hf-save').onclick = async () => {
      ...
      Object.assign(h, { nombre, tipo, frecuencia, objetivo, unidad, acumulativo: wrap.querySelector('#hf-acumular').checked, color: color.startsWith('rgb') ? rgbToHex(color) : color });
  ...
  async openHabitDetail(habit) {
    ...
    const wrap = Utils.el(`
      <div>
        ...
        <div class="stat-grid">
          <div class="stat-box"><div class="val"><span class="icon-inline" style="width:16px;height:16px;">${Icon.flame()}</span> ${streak.current}</div><div class="lbl">Racha actual</div></div>
